# M07.5 - Alternatives to Least-Squares and Ridge Regression (FIXED)

source("myladlasso_fixed.R")
...
# Least Squares
b_ols = myladlasso_fixed(Xk, Y, lam = 0, method = "ols")
Yhat = X %*% b_ols
res = Y - Yhat
SSE = sum(res^2)
RMSE_LS = sqrt(SSE / n)
...
# Ridge
b_ridge = myladlasso_fixed(Xk, Y, lam = 15, method = "ridge")
Yhat = X %*% b_ridge
res = Y - Yhat
SSE = sum(res^2)
...
# LAD
b_lad = myladlasso_fixed(Xk, Y, lam = 0, method = "lad")
Yhat = X %*% b_lad
res = Y - Yhat
SSE = sum(res^2)
RMSE_LAD = sqrt(SSE / n)
...
# Lasso
b_lasso = myladlasso_fixed(Xk, Y, lam = 15, method = "lasso")
Yhat = X %*% b_lasso
res = Y - Yhat
SSE = sum(res^2)
...
# LAD-Lasso
b_ladlasso = myladlasso_fixed(Xk, Y, lam = 15, method = "ladlasso")
Yhat = X %*% b_ladlasso
res = Y - Yhat
SSE = sum(res^2)
...
cat("\n*** FIXED VERSION (not penalizing intercept) ***\n")
print(results)
